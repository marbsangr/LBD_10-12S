...
CREATE VIEW Vista_R
AS SELECT Nom_Restaurante, Sucursal_Ubicacion
FROM Restaurante;
...
CREATE VIEW Vista_Mesa
AS SELECT Seccion_Restaurante
FROM Mesa;

SELECT * FROM Vista_Mesa;
...
CREATE VIEW Vista_P2
AS SELECT Nom_Personal, Id_Mesa, Tipo_Personal
FROM Tipo_Personal;

SELECT * FROM Vista_P2;

SELECT * FROM Personal
WHERE Id_Turno IN (SELECT Id_Turno FROM Personal)
...
CREATE VIEW Vista_Turno
AS SELECT 
FROM Turno;

SELECT * FROM Vista_Turno;
...
CREATE VIEW Vista_Cli
AS SELECT Usuario_Cliente, Notas_Cliente
FROM Cliente;

SELECT * FROM Vista_Cli;
...
CREATE VIEW Vista_Bebida
AS SELECT Nom_Bebida, Precio_Bebida
FROM Bebida;

SELECT * FROM Vista_Bebida;
...
CREATE VIEW Vista_Com
AS SELECT ClienteP, PlatilloP, Fecha_Pedido
FROM Comanda;

SELECT * FROM Vista_Com;
...
CREATE TRIGGER T1_Bebida
ON Mesa FOR INSERT AS
BEGIN 
 DECLARE @Mesa AS INT
 DECLARE @Nom_Bebida AS INT

 SET @Mesa =(SELECT id_Mesa FROM INSERTED)
 SET @Nom_Bebida =(SELECT Nom_Bebida FROM INSERTED)

 UPDATE Comanda
 SET Ruta_Pedido = Ruta_Pedido + @Nom_Bebida
 WHERE Id_Mesa LIKE @Mesa

 END

CREATE TRIGGER T2_Cliente
ON Mesa FOR INSERT AS
BEGIN 
DECLARE @Mesa AS INT
DECLARE @Notas_Cliente AS INT

SET @Mesa =(SELECT id_Mesa FROM INSERTED)
SET @Notas_Cliente =(SELECT Notas_Cliente FROM INSERTED)

UPDATE Comanda
SET Ruta_Pedido = Ruta_Pedido + Notas_Cliente
WHERE Id_Mesa LIKE @Mesa
...
AS 
SELECT * FROM Restaurante
WHERE Nom_Restaurante = 'Restaurante Bisontes'
...
AS 
SELECT * FROM Mesa
WHERE Seccion_Restaurante = 'Terraza'
...
AS 
SELECT * FROM Cliente
WHERE Notas_Cliente = 'Tacos sin piña'
...
AS 
SELECT * FROM Personal
WHERE Nom_Personal = 'Ana Cristina'
